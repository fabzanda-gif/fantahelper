    ...
    if rosters_data:
        ...
        for _, row in edited_df.iterrows():
            ...
            if row["⭐ Preferito"]:
                ...
                st.session_state.preferred_players.discard(p_id)

        st.divider()
        st.subheader("🏆 Classifica e Voto Asta per Squadra")
        st.markdown("Il voto dell'asta unisce: **1)** Rating medio della rosa, **2)** Gestione economica (differenza listino vs spesa totale), **3)** Penalizzazione per criticità e rischi rosa.")
        ...
        for t_name in team_names:
            ...
            for p in t_players:
                if p.get("role") != "P" and p.get("team_nfl"):
                    nfl_c[p["team_nfl"]] = nfl_c.get(p["team_nfl"], 0) + 1
            ...
            auction_grades.append({
                "Squadra": t_name,
                "Voto Asta": voto_asta,
                "Rating Medio": round(avg_rtg, 1),
                "Bilancio Affari (Listino - Speso)": risparmio_crediti,
                "Criticità Rilevate": critiche_count
            })
